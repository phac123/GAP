...
from data_utils.dataset import load_file, MyDataset, Label2Logit
from p_tuning.modeling import PTuneForGlue

from sklearn.metrics import matthews_corrcoef     # CoLA, AX
from sklearn.metrics import accuracy_score     # SST-2, MNLI-m, MNLI-mm, QNLI, RTE, WNLI
from sklearn.metrics import f1_score     # MRPC, QQP
from scipy.stats import pearsonr     # STS-B
import torch, gc
...
SUPPORT_MODELS = ['bert-base-uncased', 'bert-large-cased-uncased',]
SUPPORT_CLASS_NUM = [2, 3, 5]
...
def construct_generation_args():
    ...
    parser.add_argument("--out_dir", type=str, default='/raid/phac123/Model_Output/Bert-Base-HMM_GLUE/A_P/out')
    ...
    parser.add_argument("--p_epochs", type=int, default=35)
...
class Trainer(object):
    def __init__(self, args):
        ...
        if self.args.relation_id in self.seldom_input:
            ...
            self.train_set = MyDataset('train',self.train_input0, None, self.train_label, self.tokenizer, self.args)
            self.dev_set = MyDataset('dev', self.dev_input0, None, self.dev_label, self.tokenizer, self.args)
            self.test_set = MyDataset('test', self.test_input0, None, None, self.tokenizer, self.args)
            ...
            elif self.args.relation_id == 'AX-m' or self.args.relation_id == 'AX-mm':
                for i in range(len(self.train_label)):
            ...
            elif self.args.relation_id == 'MNLI-m' or self.args.relation_id == 'MNLI-mm':
                for i in range(len(self.train_label)):
                    self.train_label_logit.append(tmp_la2lo.MNLI_dict_la2i[self.train_label[i]])
                for i in range(len(self.dev_label)):
                    self.dev_label_logit.append(tmp_la2lo.MNLI_dict_la2i[self.dev_label[i]])
            elif self.args.relation_id == 'QNLI':
                for i in range(len(self.train_label)):
                    self.train_label_logit.append(tmp_la2lo.QNLI_dict_la2i[self.train_label[i]])
                for i in range(len(self.dev_label)):
                    self.dev_label_logit.append(tmp_la2lo.QNLI_dict_la2i[self.dev_label[i]])
            else:
                for i in range(len(self.train_label)):
            ...
            self.train_set = MyDataset('train', self.train_input0, self.train_input1, self.train_label_logit, self.tokenizer, self.args)
            self.dev_set = MyDataset('dev', self.dev_input0, self.dev_input1, self.dev_label_logit, self.tokenizer, self.args)
            self.test_set = MyDataset('test', self.test_input0, self.test_input1, None, self.tokenizer, self.args)
            os.makedirs(self.get_save_path(), exist_ok=True)
    ...
    def get_checkpoint(self, epoch_idx, dev_score, dev_loss, test_loss):
        if self.args.relation_id in ['SST-2', 'MNLI-m', 'MNLI-mm', 'QNLI', 'RTE', 'WNLI']:
            ckpt_name = "epoch_{}_dev_score_{}_dev_loss{}_test_loss{}.ckpt".format(epoch_idx, round(dev_score.item() * 100, 4), round(dev_loss, 4), round(test_loss, 4))
        else:
            ckpt_name = "epoch_{}_dev_score_{}_dev_loss{}_test_loss{}.ckpt".format(epoch_idx, round(dev_score * 100, 4), round(dev_loss, 4), round(test_loss, 4))
        return {
            'partial_state_dict': {
                **self.model.prompt_encoder.state_dict(),
                **self.model.transition_function.state_dict(),
                **{k: v for k, v in self.model.model.state_dict().items() if 'lora' in k or 'classifier' in k}
            },
            'embedding': self.model.prompt_encoder.state_dict(),
            'classifier': self.model.get_classifier_state_dict(),
            'dev_score': dev_score,
            'dev_loss': dev_loss,
            'test_loss': test_loss,
    ...
    def evaluate(self, epoch_idx, evaluate_type):
        ...
        with torch.no_grad():
            ...
            for input0, input1, label in loader:
                preds_y = self.model(input0, input1, label)
                preds_y = preds_y.to('cpu')
                label = torch.round(label).long()
                if self.args.relation_id == 'STSB':
                    preds_y = sigmoid(preds_y) * 5
                ...
                if evaluate_type == 'Test':
                    ...
                    elif self.args.relation_id in ['MRPC', 'QQP']:
                        preds_y = np.argmax(preds_y, axis = 1).flatten()
                        label = label.flatten()
                        score.append(f1_score(preds_y, label, average='macro'))
                    elif self.args.relation_id in ['CoLA', 'AX-m', 'AX-mm']:
                        preds_y = np.argmax(preds_y, axis = 1).flatten()
                        label = label.flatten()
                        score.append(matthews_corrcoef(preds_y, label))
                    else:
                        preds_y = preds_y.squeeze()
                        label = label.flatten()
                        score.append(pearsonr(preds_y, label).statistic)
    ...
    def gan_train(self):
        ...
        for i in range(self.args.gan_epochs):
            ...
            epochs = self.args.a_epochs
            self.args.lr = self.args.a_lr
            self.model.print_trainable_parameters(self.model)
            permt_best_ckpt, permt_best_dev_score, permt_model_path = self.train('Train: A', i, epochs, best_dev_score, best_ckpt)
            ...
            print("best_dev_score:{}, best_model_save_path:{}.".format(best_dev_score, best_model_save_path))
            ...
            epochs = self.args.p_epochs
            self.args.lr = self.args.p_lr
            self.model.print_trainable_parameters(self.model)
            permt_best_ckpt, permt_best_dev_score, permt_model_path = self.train('Train: P', i, epochs, best_dev_score, best_ckpt)
            if permt_best_dev_score > best_dev_score:
                best_dev_score = permt_best_dev_score
                best_model_save_path = permt_model_path
                best_ckpt = permt_best_ckpt
    ...
    def train(self, train_type, outer_epoch, epochs, best_dev_score, best_ckpt):
        ...
        for epoch_idx in range(epochs):
            ...
            for batch_idx, batch in tqdm(enumerate(self.train_loader)):
                self.model.train()
                optimizer.zero_grad()
                predict_y = self.model(batch[0], batch[1], batch[2])
                if self.args.relation_id == 'STSB':
                    predict_y = sigmoid(predict_y) * 5
                    predict_y = predict_y.squeeze()
                    predict_y = predict_y.float()
                    batch[2] = batch[2].float()
            ...
            if dev_score > best_dev_score:
                ...
                test_loss, test_score = self.evaluate(epoch_idx, 'Test')
                best_ckpt = self.get_checkpoint(epoch_idx, dev_score, dev_loss, test_loss)
                early_stop = 0
